...
set -euo pipefail
: "$VLLM_IMAGE" "$GS_MODEL" "$HF_CACHE_DIR"
...
prep() { # $1 = ssh fn name, $2 = label
  ...
  # NB: k3s runs its own embedded containerd (/run/k3s/containerd/); the
  # system ctr pulls into a store the kubelet never sees. k3s crictl hits
  # the right CRI socket by construction. Requires 02-agent done first.
  "$1" "sudo k3s crictl pull 'docker.io/$VLLM_IMAGE' \
    && sudo mkdir -p '$HF_CACHE_DIR' \
    && { sudo python3 -m pip install -q huggingface_hub || sudo python3 -m pip install -q --break-system-packages huggingface_hub; } \
    && sudo -- sh -c 'command -v hf' \
    && sudo HF_HOME='$HF_CACHE_DIR' hf download '$GS_MODEL' \
    && echo '=== $2 DONE ==='" &
}
