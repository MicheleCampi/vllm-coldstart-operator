...
export GS_SSH_KEY="${GS_SSH_KEY:-$HOME/.ssh/lambda}"   # key used for ubuntu@<ip>
...
export GS_MODEL="Qwen/Qwen2.5-7B-Instruct"
export HF_CACHE_DIR="/opt/hf-cache"

ssh_a() { ssh -i "$GS_SSH_KEY" -o StrictHostKeyChecking=accept-new "ubuntu@$NODE_A_IP" "$@"; }
ssh_b() { ssh -i "$GS_SSH_KEY" -o StrictHostKeyChecking=accept-new "ubuntu@$NODE_B_IP" "$@"; }
ssh_c() { ssh -i "$GS_SSH_KEY" -o StrictHostKeyChecking=accept-new "ubuntu@$NODE_C_IP" "$@"; }
# Propagate the ssh helpers to child bash processes (the numbered scripts
# are executed, not sourced). GS_ prefix on generic names (MODEL, SSH_KEY)
# avoids collisions with other harnesses (e.g. hack/rehearsal/run.sh).
export -f ssh_a ssh_b ssh_c
